...
bool dynamic_allocator_create(u64 total_size, u64* memory_requirement, void* memory, dynamic_allocator* out_allocator) {
    if (total_size < 1) {
    ...
    *memory_requirement = freelist_requirement + sizeof(dynamic_allocator_state) + total_size;
    ...
    Dzero_memory(state->memory_block, total_size);
    return true;
}
...
bool dynamic_allocator_destroy(dynamic_allocator* allocator) {
    if (allocator) {
        ...
        Dzero_memory(state->memory_block, state->total_size);
        state->total_size = 0;
        allocator->memory = 0;
        return true;
    }

    DWARNING("dynamic_allocator_destroy requires a pointer to an allocator. Destroy failed.");
    return false;
}
...
void* dynamic_allocator_allocate(dynamic_allocator* allocator, u64 size) {
      if (allocator && size) {
        ...
        if(freelist_allocate_block(&state->list, size, &offset)){
            ...
            DERROR("dynamic_alllocator_allocate no blocks of memory large enough to allocate from");
            u64 available = freelist_free_space(&state->list);
            DERROR("requested size: %llu total space available: %llu", size, available);
            return nullptr;
        }
      }
      DERROR("dynamic_allocator_free requires both a valid allocator and size");
      return nullptr;
}     
...
//         DASSERT_MSG(required_size < 4294967295U, "dynamic_allocator_allocate_aligned called with required size > 4 GiB. Don't do that.");
...
//         } else {
//             DERROR("dynamic_allocator_allocate_aligned no blocks of memory large enough to allocate from.");
//             u64 available = freelist_free_space(&state->list);
//             DERROR("Requested size: %llu, total space available: %llu", size, available);
...
//     DERROR("dynamic_allocator_allocate_aligned requires a valid allocator, size and alignment.");
//     return 0;
// }
...
bool dynamic_allocator_free(dynamic_allocator* allocator, void* block, u64 size) {
    ...
    if(block <  state->memory_block || block > ((char*)state->memory_block+ state->total_size)){
        ...
        DERROR("dynamic_allactor_free trying to release block (0x%p) outside of allocator range (0x%p)-(0x%p)", block,state->memory_block,end_of_block);
        return false;
    }
...
// bool dynamic_allocator_free_aligned(dynamic_allocator* allocator, void* block) {
//     if (!allocator || !block) {
//         DERROR("dynamic_allocator_free_aligned requires both a valid allocator (0x%p) and a block (0x%p) to be freed.", allocator, block);
//         return false;
//     }
...
//         DWARNING("dynamic_allocator_free_aligned trying to release block (0x%p) outside of allocator range (0x%p)-(0x%p)", block, state->memory_block, end_of_block);
//         return false;
//     }
...
//         DERROR("dynamic_allocator_free_aligned failed.");
//         return false;
//     }
...
// bool dynamic_allocator_get_size_alignment(void* block, u64* out_size, u16* out_alignment) {
//     // Get the header.
