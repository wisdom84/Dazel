...
void freelist_create(u64 total_size, u64* memory_requirement, void* memory, freelist* out_list) {
    ...
    *memory_requirement = sizeof(internal_state) + (sizeof(freelist_node) * max_entries);
    ...
    Dzero_memory(out_list->memory, *memory_requirement);
    internal_state* state = (internal_state*)out_list->memory;
    state->nodes = (freelist_node*)((char*)out_list->memory + sizeof(internal_state));
    state->max_entries = max_entries;
    state->total_size = total_size;

    Dzero_memory(state->nodes, sizeof(freelist_node) * state->max_entries);
...
void freelist_destroy(freelist* list) {
    if (list && list->memory) {
        ...
        Dzero_memory(list->memory, sizeof(internal_state) + sizeof(freelist_node) * state->max_entries);
        list->memory = 0;
    }
}
...
bool freelist_allocate_block(freelist* list, u64 size, u64* out_offset) {
    ...
    DWARNING("freelist_find_block, no block with enough free space found (requested: %lluB, available: %lluB).", size, free_space);
    return false;
}
...
bool freelist_free_block(freelist* list, u64 size, u64 offset) {
    ...
    if (!node) {
        ...
        while (node) {
            if (node->offset + node->size == offset) {
                ...
                DFATAL("Attempting to free already-freed block of memory at offset %llu", node->offset);
                return false;
            } else if (node->offset > offset) {
    ...
    DWARNING("Unable to find block to be freed. Corruption possible?");
    return false;
}

bool freelist_resize(freelist* list, u64* memory_requirement, void* new_memory, u64 new_size, void** out_old_memory) {
    ...
    *out_old_memory = list->memory;
    ...
    // The block's layout is head* first, then array of available nodes.
    Dzero_memory(list->memory, *memory_requirement);

    // Setup the new state.
    internal_state* state = (internal_state*)list->memory;
    state->nodes = (freelist_node*)((char*)list->memory + sizeof(internal_state));
    state->max_entries = max_entries;
